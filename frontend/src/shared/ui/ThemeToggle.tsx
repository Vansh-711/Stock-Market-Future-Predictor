...
export const ThemeToggle = () => {
  ...
  const [isDark, setIsDark] = useState(() => {
    ...
    return true; // Default to dark mode if nothing saved
  });

  const buttonRef = useRef<HTMLDivElement>(null);
  const isTransitioningRef = useRef(false);

  // Sync initial state to DOM without transition
  useEffect(() => {
  ...
  const toggleTheme = useCallback((e: React.MouseEvent) => {
    ...
    const toX = (nx: number) => `${(nx / viewportWidth) * 100}%`;
    const toY = (ny: number) => `${(ny / viewportHeight) * 100}%`;
    const point = (nx: number, ny: number) => `${toX(nx)} ${toY(ny)}`;
  ...
  return (
    <div ref={buttonRef} className="theme-toggle-wrapper fixed top-6 right-6 z-[100]" onClick={toggleTheme}>
      <label className="theme-switch" style={{ pointerEvents: 'none' }}>
        <input 
          type="checkbox" 
